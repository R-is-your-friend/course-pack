...
You can create an RStudio project with a new directory or add a project to an already existing directory. When you convert a directory into a RStudio project, you will notice that a `.Rproj` file now exists in that directory. 

If you are primarily writing code in R scripts (as you have done so far in this class), a major benefit of RStudio Projects is that they **set** the working directory as the main project directory. For example:


However, if you are working in a Quarto file (.qmd), the working directory is the directory where the Quarto file lives! This can be tricky -- just remember that Quarto wins ☺️. 

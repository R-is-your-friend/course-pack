...
1. What is the **name** of this function?  e.g. `sqrt()`, `case_when()`, `median()`. 
Remember that the name was chosen by the human that wrote that code - you can 
usually guess what the function will do from the name, but you can't know for
sure without reading the *documentation*!

2. What are the **arguments** of this function?  That is, what information do you,
the user, have to give to the function to make it calculate something for you?
For example, `median(x = 1:10, na.rm = TRUE)` has two arguments: `x`, which supplies
the *vector* we want the median value of, and `na.rm`, which tells the function how to 
handle missing data.

3. What **package** does this function come from?  You may have to load an external
library, such as `dplyr` to be able to use the function.



### Function Arguments

So far in this course, we've been modeling how we believe functions should be used. 
Specifically, we've been using **named arguments** when calling the function for
every argument except the very first one.
(e.g., `median(1:10, na.rm = TRUE)`). Technically, 
...
Uh oh! Why did we get a strange error here? Well, `median()` 
is assuming the second argument is associated with `na.rm`. So, R thinks 
`na.rm = 1:10` and `x = TRUE`. However, `na.rm` needs to be `TRUE/FALSE`, not a
numeric vector, so the function breaks!
...
::: callout-check-in
Suppose `divide()` is a function with three arguments: `x`, a number; `y`, a 
number; and `round`, a `TRUE/FALSE` value.
The function calculates `x/y` and then possibly rounds the result if `round`
is `TRUE`.

We want to calculate `17/3` and then round the result.
For each of the *function calls* below, indicate if this would result in
...
5.  What would you expect `rescale()` to output when `test_single` is input?
6.  What would you expect `rescale()` to output when `test_missing` is input?
7.  What would you expect `rescale()` to output when `test_type` is input?
:::
...
::: callout-check-in
Think about some ways that the `tax_rate` function 

8.  What would you expect `tax_rate()` to output when a *character* object is input?  Why?
9.  What would you expect `tax_rate()` to output when no arguments are provided?  Why?
10.  What would you expect `tax_rate()` to output when a numeric vector is input? Why?
:::
...
`rescale()` does take a vector as input, but it applies a process to *the whole vector at once*, not
...
As for `tax_rate` we have already seen in previous readings that we get the error message 
...
for(i in 1:3) {                                       # Q11
  ...
  economy_data[[i]] <- rescale(economy_data[[i]])     # Q12

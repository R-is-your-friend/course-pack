...
4.  What would you expect `tax_rate()` to output when a *character* object is input?  Why?
5.  What would you expect `tax_rate()` to output when no arguments are provided?  Why?
6.  What would you expect `tax_rate()` to output when a numeric vector is input? Why?
:::
...
::: callout-check-in
7,8,9. Suppose we want to use a *loop* to rescale the vectors `cpi`, `ppe`, 
...
for(i in 1:3) {
  ...
  economy_data[[i]] <- rescale(economy_data[[i]])

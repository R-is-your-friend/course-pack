...
<!-- [WEBR?] -->
<!-- The code below could just be included in a code chunk so the students can run -->
<!-- it to prove to themselves that the output will be 0.22. -->
...
::: {.callout-check-in}
3. In the IRS tax brackets we had ____ possible outcomes and ____ conditions
...
evaluated because `income` has a length that is greater than 1.
If we examine `income < 11925`, we notice that it returns a vector of three 
`TRUE/FALSE` results; one for each of the three incomes.

```{r}
#| label: conditional-with-vector-2

income < 11925
```

Therefore, our code was equivalent to `if(c(FALSE, FALSE, TRUE))`.  An `if()`
statement needs a single "yes/no" to determine if the code inside gets run or
not, so these statements can only work for scalar vectors (i.e., vectors with a
single value)

## Vectorized Operations 

So, what do we do if we want to apply a process using conditional statements 
to *all* the elements in a vector of values? Well, we need to switch from using 
`if ()`, `else if ()`, and `else` to using functions that accept vectors of values. 
...
**vector** of values, applies the square root calculation to *each value*, and
outputs a vector of the results. 
...
### Vectorized Conditional Statements {#sec-3a-case-when}
...
[equal to / greater than / less than] 1 as an input and applies a process
to every value. 
...
The `case_when()` function comes from the **dplyr** package which you will learn
more about later on in the course. You might be wondering what we mean when we
say "R package"? That's a great question!

R as a programming language is comprised of packages. Each package contains 
collections of code, data, documentation, and tests and is housed in a way that
makes it easy to share with others. Since Day 1, we've been using the **base**
package, which contains the basic functions which let R function as a language:
arithmetic, input/output, basic programming support. The base package is loaded
by default, so we would have never known that a package had been loaded! 

R packages extend the capabilities of R by adding functionality to address 
specific problems. The package-based system is what makes R so successful.
Chances are someone else has already solved the problem you are working on and 
you can benefit from the code they made to solve it! 

Many R packages are hosted on the Comprehensive R Archive Network (CRAN). As of
July 2026, the [CRAN has 24,216 packages](https://cran.r-project.org/web/packages/).

## Help tab

You might be wondering, how would I have figured out the syntax for
`case_when()` on my own? No! Let's introduce you to the Help tab---your friend
for getting help with how to use an R function.

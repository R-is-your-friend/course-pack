...
::: {.callout-check-in}
3. In the IRS tax brackets we had [one / two / three / four] possible outcomes
and [one / two / three / four] conditions that needed to be checked. 

4. In general, if you have $n$ possible outcomes, you will **always** have
[$n$ / $n + 1$ / $n - 1$ / $n * 2$] conditions to check. 
:::
...
::: {.callout-check-in}
7. What does the `.unmatched` argument of `case_when()` do?

8. What does the `.default` argument of `case_when()` do?

9. Can you use **both** the `.unmatched` and `.default` arguments? Meaning
would it work to set `.unmatched =  "error"` and set `.default = "zero"`?
